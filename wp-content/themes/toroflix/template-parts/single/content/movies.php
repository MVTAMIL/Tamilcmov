...
<article class="TPost A Single">
    ...
    <footer>
        ...
        if($trailer_field){
        ?>
        <button type="button" class="Button TPlay AAIco-play_circle_outline" data-trailer="<?php echo htmlentities($trailer_field); ?>"><?php printf( __('Watch Trailer', 'toroflix'), '<strong>', '</strong>' ); ?></button>
        <?php } ?>
